...
- 导读页只维护栏目级阅读方向，不维护具体文章清单；文章之间有强关联时才在正文互链
- 博客规划按增量系统处理：新增普通文章时，原则上只新增文章本身，不要求同步修改导航、导读、维护页或其他无关文章
- 优先使用稳定的分类页承接增长，避免手工维护文章索引；只有系列、前置依赖、直接续篇才做明确互链
- 明显只是占位的文章不要长期公开挂在首页
